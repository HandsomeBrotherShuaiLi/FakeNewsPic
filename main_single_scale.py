...
class Mymodel(object):
    ...
    def ensemble(self,model_path_list,mode='vote',test_image_dir=None):
        ...
        if mode=='vote':
            ...
            for i in range(len(model_path_list)):
                ...
                if train_prob_file_name not in files:
                    ...
                    for line in train_prob_file:
                        if line!='\n':
                            temp = line.split(',')
                            all_train_dict[temp[0]].append(float(temp[-1].strip('\n')))
            all_data=np.zeros(shape=(len(all_train_dict),len(list(all_train_dict.values())[0])+1))
            print(all_data.shape)
            s=set()
            for i in range(len(all_train_dict)):
                s.add(len(all_train_dict[list(all_train_dict.keys())[i]]))
            print(s)
            return
                # all_data[i,:-1]=all_train_dict[list(all_train_dict.keys())[i]]
                # all_data[i,-1]=float(all_labels[list(all_train_dict.keys())[i]])
            ...
            X_train, X_test, y_train, y_test = train_test_split(all_data[:,:-1], all_data[:,-1], test_size=0.3, random_state=2019)
